...
with st.sidebar:
    ...
    show_lines = st.sidebar.checkbox("線を表示", value=True, key="show_lines")
...
def analyze_and_plot(df, title, x_col):
    show_lines = st.session_state.get("show_lines", True)
    ...
    # プロット方法の切り替え
    mode_normal = 'lines+markers' if show_lines else 'markers'

    df_normal = df[~df["超過フラグ"]]
    df_over = df[df["超過フラグ"]]

    fig.add_trace(go.Scatter(
        x=df_normal[x_col], y=df_normal["1時間前までの件数"],
        mode=mode_normal, name="正常", marker=dict(color="blue", size=5),
        hovertemplate="日時: %{x}<br>件数: %{y}"
    ))

    fig.add_trace(go.Scatter(
        x=df_over[x_col], y=df_over["1時間前までの件数"],
        mode="markers", name="超過", marker=dict(color="red", size=7),
        hovertemplate="日時: %{x}<br>件数: %{y}"
    ))
...
def summarize_peak(df_result):
    ...
    peak_rows = df_result[df_result["1時間前までの件数"] == max_val]

    if peak_rows.empty or peak_rows["リクエスト日時"].isnull().all():
        st.info("📉 ピーク時刻が無効です。")
        return

    peak_time = peak_rows.iloc[0]["リクエスト日時"]
